...
wen 自 0.4.0 起拆为 cjpm **workspace** 两个子包:

- **`wen`**(core) — 路由、请求/响应、解析、流式、文件、会话、ETag 等协议层 + 数据流必需。
- **`wen_contrib`**(可选)— `helmet` / `csrf` / `cors` / `basicAuth` / `bearerAuth` /
  `logger` / `requestId` / `methodOverride` / `methodOverrideField` 等随安全策略和
  行业惯例频繁迭代的中间件。**只用 wen 不引这些** — 不为没用到的能力背锁定。
...
[dependencies]
  ...
  # 可选:contrib 中间件(若用到 helmet / csrf / cors / auth / logger / requestId / methodOverride)
  wen_contrib = { git = "https://github.com/mtpkiss/wen.git", branch = "main", path = "wen-contrib" }
```

> 设计理由:Express 4 → 5 把 `body-parser` / `cookie-parser` / `csurf` 移出 core
> 是因为安全策略变化频繁,绑死 core 后小补丁要等框架 release。wen 1.0 锁定前
> 一次性把"会随安全态势 / 行业惯例频繁迭代"的中间件拆到 `wen_contrib`,core 收敛
> 到协议 + 数据流必需的最小集。
...
> 真"最简"无中间件 — 这能让你判断 wen 主包 API 本身的复杂度,而非把 contrib
> 的便利特性混在第一印象里。需要 `logger()` / `cors()` 等,加 `import wen_contrib.*`,
> 见下方完整示例。
...
import wen.*
import wen_contrib.*   // logger / helmet / cors / csrf / auth / requestId / methodOverride 在此

main(): Int64 {
    let app = wen()

    // ---- 全局中间件 ---------------------------------------------------------
    app.use(logger())           // [wen_contrib]
    app.use(helmet())           // [wen_contrib] 安全响应头(CSP / nosniff / HSTS / 防点击劫持等)
...
- **中间件**:中间件链 `(req, res, next)`、全局/路径挂载、数组批量挂载、错误处理中间件
  `(err, req, res, next)`。
    - `wen` core 内置:`etag` / `staticFiles` / `session` / `multipart`;JSON / 表单 /
      Cookie 解析已**内置**(`req.json()/attribute()/cookie()` 首次访问即自动解析,无需注册)。
    - `wen_contrib` 可选:`logger / requestId / cors / helmet / basicAuth / bearerAuth
      / csrf / methodOverride / methodOverrideField` — 单独依赖 `wen_contrib` 包后
      `import wen_contrib.*` 使用。
...
| `multipart(...)` | multipart 解析显式配额(`maxFileSize` / `maxFiles` / `maxFields` / `fileFilter`) |

### `wen_contrib`(可选,需单独依赖)
